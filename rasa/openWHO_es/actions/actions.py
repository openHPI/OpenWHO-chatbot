...
class ActionGetCourses(Action):
	...
	def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
		...
		if status == 200:
			...
			if len(response) < 1:
				dispatcher.utter_message('Actualmente no está matriculado en ningún curso.')
				return [SlotSet('courses_available', False)]
			else:
				dispatcher.utter_message('Actualmente está matriculado en estos cursos:')
				buttonGroup = []
...
class ActionGetAchievements(Action):
	...
	def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
		...
		if courseId != 0:	
			...
			dispatcher.utter_message('Lo siento mucho. No he podido encontrar el curso que busca. Por favor, inténtelo de nuevo indicándome el título del curso.')
			return[SlotSet('current_course_achieved', course_achieved), 
			SlotSet('current_course', currentCourse), 
			SlotSet('current_achievements', currentAchievements), 
			SlotSet('current_course_title', None)]
		...
		for achievement in currentAchievements:
			if achievement['achieved']:
				if achievement['download']['available']:
					...
					dispatcher.utter_message('Lo siento mucho. ¡El {0} ya no está disponible y lamentablemente ya no se puede descargar!'.format(achievement['name']))
		return []
